function btn1(){
  document.getElementById("changecolor").style="background-color:#e0e0e0";
  document.getElementById("box1").innerHTML="#e0e0e0"
}
function bt n2(){
  document.getElementById("changecolor").style="background-color:#6fcf97"
  document.getElementById("box1").innerHTML="#6fcf97"
}
function btn3(){
 document.getElementById("changecolor").style="background-color:#56ccf2";
 document.getElementById("box1").innerHTML="#56ccf2"
}
function btn4(){
    document.getElementById("changecolor").style="background-color:#bb6bd9";
    document.getElementById("box1").innerHTML="#bb6fd9"
   }
